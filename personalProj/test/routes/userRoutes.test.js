...
describe('GET /users', () => {
    ...
    userProperties = Object.getOwnPropertyNames(testUser).filter(prop =>
        typeof testUser[prop] !== 'function' && prop !== 'password'
    );
...
describe('GET /users/:id', () => {
    test('should return a user from DB', async () => {
        ...
        expect(res.statusCode).toEqual(200);
        Object.keys(userFields).forEach(key => expect(res.body[key]).toEqual(userFields[key]));
        userProperties.forEach(prop => expect(res.body).toHaveProperty(prop));
    });
